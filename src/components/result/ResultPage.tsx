...
import { PHOTO_STORAGE_KEY } from "@/components/photobooth/Photobooth";
import { uploadPhoto } from "@/lib/uploadPhoto";

export const PHOTO_URL_KEY = "photoboothUploadedUrl";
...
export function ResultPage() {
  ...
  const runUpload = useCallback(
    async (dataUrl: string, signal?: AbortSignal) => {
      ...
      try {
        ...
        try {
          sessionStorage.setItem(PHOTO_URL_KEY, res.file);
        } catch {
          // ignore
        }
  ...
  useEffect(() => {
    ...
    try {
      dataUrl = sessionStorage.getItem(PHOTO_STORAGE_KEY);
      cachedUploadUrl = sessionStorage.getItem(PHOTO_URL_KEY);
    } catch {
      dataUrl = null;
    }
    if (!dataUrl) {
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 flex h-full w-full flex-col items-center justify-center px-[6cqw] py-[6cqw]">
          ...
          <Image
            src="/images/I-SCAN.png"
            alt="Scan QR to download"
            width={1000}
            height={180}
            priority
            className="mt-[3cqw] h-auto w-[70vw] object-contain"
          />

          <div className="mt-[5cqw] flex aspect-square w-[60vw] items-center justify-center rounded-xl bg-white p-[2cqw] shadow-xl">
            {status === "uploading" && (
          ...
          <Image
            src="/images/DWP-BACK.png"
            alt="Back to home"
            width={1000}
            height={180}
            priority
            className="pointer-events-none z-20 mt-[5cqw] h-auto w-[50vw] object-contain"
          />
        </div>
      </div>
    </Link>
