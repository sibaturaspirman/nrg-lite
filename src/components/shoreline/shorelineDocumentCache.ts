import { SHORELINE_ROUTES } from "@/app/shoreline-sw-routes";

const NAV_CACHE = "shoreline-navigations";
 ...
 */
export async function persistShorelineDocuments(): Promise<void> {
  ...
  try {
    ...
    await Promise.all(
      SHORELINE_ROUTES.map(async (path) => {
        try {
