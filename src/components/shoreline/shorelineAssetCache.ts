import { SHORELINE_ASSETS } from "@/components/shoreline/shorelineNav";
...
export function subscribeShorelineAssets(listener: () => void): () => void {
...
export function resolveShorelineAsset(src: string): string {
  if (src.startsWith("data:") || src.startsWith("blob:")) return src;
  return blobUrls.get(src) ?? src;
}

export function warmShorelineAssets(): Promise<void> {
  if (typeof window === "undefined") return Promise.resolve();
  if (warming) return warming;

  warming = (async () => {
    await Promise.all(
      SHORELINE_ASSETS.map(async (path) => {
