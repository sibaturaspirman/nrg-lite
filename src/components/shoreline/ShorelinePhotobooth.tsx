...
import { setShorelineShots } from "@/components/shoreline/shorelineSession";
import {
  CAPTURE_H,
  CAPTURE_RATIO,
  CAPTURE_W,
  compositeFramedCapture,
  warmStripAssets,
} from "@/components/shoreline/shorelineStrip";
...
const FRAME_SRC = "/images/shoreline/template.png";

export const SHORELINE_PHOTO_KEY = "shorelineShot";
export const SHORELINE_PHOTO_URL_KEY = "shorelineUploadedUrl";
export const SHORELINE_SHOTS_KEY = "shorelineShots";
export const SHORELINE_PRINT_KEY = "shorelinePrint";
export const SHORELINE_TEMPLATE_INDEX_KEY = "shorelineTemplateIndex";
...
export function ShorelinePhotobooth() {
  ...
  const [shots, setShots] = useState<(string | null)[]>([null, null]);
  ...
  const finishSession = useCallback((finalShots: string[]) => {
    setPreparing(true);
    try {
      sessionStorage.removeItem(SHORELINE_PHOTO_URL_KEY);
      sessionStorage.removeItem(SHORELINE_TEMPLATE_INDEX_KEY);
      sessionStorage.removeItem(SHORELINE_PHOTO_KEY);
      sessionStorage.removeItem("shorelineZynSessionCurrent");
    } catch {
      // ignore
    }
    setShorelineShots(finalShots);
    shorelinePush("/shoreline/result");
  }, []);
  ...
  const takePhoto = useCallback(async () => {
    ...
    const raw = captureFrame(video, mirror);
    if (!raw) {
    ...
    let framed: string;
    try {
      framed = await compositeFramedCapture(raw, FRAME_SRC);
    } catch {
      capturingRef.current = false;
      setBusy(false);
      setTimeout(() => setFlash(false), 180);
    ...
    <div className="flex min-h-[100dvh] w-full items-center justify-center overflow-hidden bg-black">
      ...
      >
        ...
        <div className="absolute inset-[2.8cqw] z-10 flex flex-col items-center px-[5cqw] pb-[10cqw] pt-[10cqw]">
          <Image
            src="/images/shoreline/title-booth.png"
          ...
          <div
            className="relative mt-[5.5cqw] w-full overflow-hidden bg-black"
          ...
          <div className="mt-[4cqw] grid w-[60%] grid-cols-2 gap-[2cqw]">
            {shots.map((shot, i) => (
              <div
                key={i}
          ...
          >
            <Image
              src="/images/shoreline/btn-capture.png"
              alt="Capture"
              width={1000}
              height={160}
              priority
        ...
        {error &&
          !/play\(\) request was interrupted/i.test(error) && (
