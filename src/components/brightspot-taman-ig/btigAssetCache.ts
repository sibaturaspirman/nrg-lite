import { BTIG_ASSETS } from "@/components/brightspot-taman-ig/btigNav";
...
export function subscribeBtigAssets(listener: () => void): () => void {
...
export function resolveBtigAsset(src: string): string {
  if (src.startsWith("data:") || src.startsWith("blob:")) return src;
  return blobUrls.get(src) ?? src;
}

export function warmBtigAssets(): Promise<void> {
  if (typeof window === "undefined") return Promise.resolve();
  if (warming) return warming;

  warming = (async () => {
    await Promise.all(
      BTIG_ASSETS.map(async (path) => {
