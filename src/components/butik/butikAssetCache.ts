import { BUTIK_ASSETS } from "@/components/butik/butikNav";
...
export function subscribeButikAssets(listener: () => void): () => void {
...
export function resolveButikAsset(src: string): string {
  if (src.startsWith("data:") || src.startsWith("blob:")) return src;
  return blobUrls.get(src) ?? src;
}

export function warmButikAssets(): Promise<void> {
  if (typeof window === "undefined") return Promise.resolve();
  if (warming) return warming;

  warming = (async () => {
    await Promise.all(
      BUTIK_ASSETS.map(async (path) => {
