...
export function ButikResultPage() {
  ...
  const runUpload = useCallback(
    async (dataUrl: string, signal?: AbortSignal) => {
      ...
      try {
        ...
        try {
          sessionStorage.setItem(BUTIK_PHOTO_URL_KEY, res.file);
        } catch {
          // ignore
        }
  ...
  useEffect(() => {
    ...
    try {
      cachedUploadUrl = sessionStorage.getItem(BUTIK_PHOTO_URL_KEY);
    } catch {
      cachedUploadUrl = null;
    }
    const dataUrl = getButikPhoto();
    if (!dataUrl) {
      butikReplace("/butik");
      return;
    }
